...
class ServiceStackSession(RestSession):
    ...
    def _get_operation_name(self, url, verb="GET"):
        if self.metadata is None:
            ...
            self.metadata = {route["operation"]: route["name"] for route in [item for sublist in [self._get_operation_routes(operation) for operation in self.get("/types/metadata")["Operations"]] for item in sublist]}
...
class timeseries_client:
    ...
    def getTimeSeriesUniqueId(self, timeSeriesIdentifier):
        ...
        try:
            descriptions = self.publish.get("/GetTimeSeriesDescriptionList", params={"LocationIdentifier": location})["TimeSeriesDescriptions"]
    ...
    def getLocationData(self, identifier):
        ...
        return self.publish.get("/GetLocationData", params={"LocationIdentifier": identifier})
    ...
    def getTimeSeriesDescriptions(
        self,
        locationIdentifier=None,
        parameter=None,
        publish=None,
        computationIdentifier=None,
        computationPeriodIdentifier=None,
        extendedFilters=None,
    ):
        return self.publish.get(
    ...
    def getTimeSeriesCorrectedData(
        self,
    ...
    def uploadExternalReport(self, locationUniqueId, pathToFile, title, deleteDuplicateReports=False):
        ...
        return self.acquisition.post(
            f"/locations/{locationUniqueId}/attachments/reports",
            params={"Title": title},
    ...
    def uploadFieldVisit(self, locationUniqueId, pathToFile):
        ...
        return self.acquisition.post(
            f"/locations/{locationUniqueId}/visits/upload/plugins",
            files={"file": open(pathToFile, "rb")},
        )
    ...
    def getAllLocations(self, **kwargs):
        ...
        return self.provisioning.send_batch_requests("/locations/{Id}", [{"LocationUniqueId": loc["UniqueId"]} for loc in descriptions])
    ...
    def createReflectedTimeseries(self, location_identifier_or_unique_id, series):
        ...
        response = self.provisioning.post(f"/locations/{location_unique_id}/timeseries/reflected", json=series)
    ...
    def appendReflectedPoints(self, series_identifier_or_unique_id, points, start=None, end=None):
        ...
        return self.acquisition.post(f"/timeseries/{unique_id}/reflected", json=series_data)["AppendRequestIdentifier"]
    ...
    def appendPoints(self, series_identifier_or_unique_id, points, start=None, end=None):
        ...
        if start is None and end is None:
            return self.acquisition.post(f"/timeseries/{unique_id}/append", json=series_data)["AppendRequestIdentifier"]

        series_data["TimeRange"] = {"Start": start.isoformat(), "End": end.isoformat()}

        return self.acquisition.post(f"/timeseries/{unique_id}/overwriteappend", json=series_data)["AppendRequestIdentifier"]
...
class SamplesSession(RestSession):
    ...
    def get(self, url, **kwargs):
        ...
        if self.callbacks["pagination_warning"] and json and all(key in json for key in ["totalCount", "cursor", "domainObjects"]) and json["totalCount"] > len(json["domainObjects"]):
