...
def process_reservoir_sacs(
    ...
    reservoir_sacs_daily = {staid: rcap.assign_res_sac(staid=staid, dataframe=dataframe) for staid, dataframe in reservoir_elevation_daily.items()}

    # Resample to monthly values
    reservoir_elevations_monthly = {staid: dataframe.groupby(pd.Grouper(freq="ME")).mean().round(3) for staid, dataframe in reservoir_elevation_daily.items()}
